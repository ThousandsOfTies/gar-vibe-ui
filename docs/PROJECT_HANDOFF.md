...
**❌ 取れない（公式APIなし）**：承認内容の文面（"OK to apply patch?"等）／承認待ちの正確な判定（→通知音に委ねる）／Chatパネル内の「ユーザー入力待ち」の正確な判定／AI応答テキストの取得（読み上げ以外）
...
- `idle` は「観測可能な活動がない」という意味。Chatパネル内の質問待ちは公式APIから見えないため、UI上では `quiet / unknown` として扱う。
